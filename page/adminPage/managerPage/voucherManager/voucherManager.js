...
import { createVoucher$, fetchListVoucher, removeVoucher$, updateVoucher$ } from "../../../../controllers/voucherController.js";
...
window.createVoucher = async function createVoucher() {
    try {
        ...
        const voucher = createVoucherObject(voucherNameValue, voucherAmountValue, discountValue, null);

        if (checkBoxLimitTimeDOM.checked) {
            const limitTime = Math.floor(Date.now() / 1000) + timeLimitValue * 60 * 60;
            voucher.limitTime = limitTime;
            if (validateVoucher(voucherNameValue, voucherAmountValue, discountValue) && isVoucherNameVaild) {
                if (!timeLimitValue) {
...
function createVoucherObject(voucherNameValue, voucherAmountValue, discountValue, timeLimit) {
    return new Voucher(
        voucherNameValue,
        voucherAmountValue,
        discountValue,
...
function isVoucherExisted(voucherNameInput) {
    return listVoucher.some((vouchers) => vouchers.voucherName == voucherNameInput);
}
